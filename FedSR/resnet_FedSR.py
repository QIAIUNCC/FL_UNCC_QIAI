...
class ResNetSR(ResNet):

    def __init__(self, L2R_coeff=0.01, CMI_coeff=0.001, num_samples=20, z_dim=512, **kwargs):
        """
        """
        ...
        self.encoder.fc = nn.Linear(self.model.fc.in_features, out_dim)
        self.cls = nn.Linear(z_dim, len(self.hparams.classes))
        self.r_mu = nn.Parameter(torch.zeros(len(self.hparams.classes), z_dim))
        self.r_sigma = nn.Parameter(torch.ones(len(self.hparams.classes), z_dim))
